    ...
    for c in range(COLS):
        ...
        for ship in game.player_board.ships:
            if (r, c) in ship.coordinates:
                is_ship = True
                break
        bg = COLOR_SHIP if is_ship else COLOR_WATER
        btn = PushButton(board_box_left, text=" ", grid=[r,c], width=CELL_SIZE, height=CELL_SIZE)
        btn.tk.config(bg=bg, activebackground=bg)
        own_buttons[r][c] = btn
...
def shoot(r, c):
    ...
    if result == "miss":
        ...
        btn.tk.config(bg=COLOR_HIT, activebackground=COLOR_HIT)
    elif result == "sunk":
        for (rr, cc) in game.enemy_board.ships[ship_idx].coordinates:
            opp_buttons[rr][cc].tk.config(bg=COLOR_SUNK, activebackground=COLOR_SUNK)
...
Text(app, text="Legenda: Blauw = water/miss, Grijs = eigen schip, Geel = hit, Zwart = gezonken schip", grid=[0,2,2,1])

# --- NIEUWE CODE: PLAATS-SCHEPEN MODUS + ROTATIE + HOVER PREVIEW ---
...
def clear_preview():
    for (r,c) in preview_cells:
        if player_grid[r][c] is None:
            own_buttons[r][c].tk.config(bg=COLOR_WATER, highlightbackground="#555555")
    preview_cells.clear()
...
def hover_preview(r, c):
    ...
    for (rr, cc) in cells:
        if player_grid[rr][cc] is None:
            own_buttons[rr][cc].tk.config(bg="#90EE90", highlightbackground="#00AA00")
    preview_cells[:] = cells
...
def place_ship(r, c):
    ...
    # --- correcte horizontale plaatsing ---
    else:
        if c + size > COLS:
            return
        for cc in range(c, c+size):
...
# Voorbeeld: eerst schepen plaatsen op eigen veld volgens lijst afmetingen
# (vliegdekschip 5, slagschip 4, onderzeeër 3, patrouilleschip 2)
# Dit is enkel basislogica: klikken plaatst horizontaal schip vanaf geselecteerd vak
...
# Markeer nu eigen veld als leeg voor plaatsen
for r in range(ROWS):
    for c in range(COLS):
        own_buttons[r][c].tk.config(bg=COLOR_WATER, highlightthickness=1, highlightbackground="#555555")
        player_grid[r][c] = None
...
def place_ship(r, c):
    ...
    # check horizontale plaatsing mogelijk
    if c + size > COLS:
        return
    ...
    # plaats schip
    for cc in range(c, c+size):
        own_buttons[r][cc].tk.config(bg=COLOR_SHIP, highlightthickness=1, highlightbackground="#000000")
        player_grid[r][cc] = current_place_index
